...
public class StudentRepositoryTest {
    ...
    @Test
    public void testGetFullStudentExist() throws Exception {
        System.out.println("testGetFullStudentExist");
        //Arrange
        StudentDto expected = bob;
        StudentRepository repository = new StudentRepository(mock);
        //Action
        StudentDto result = repository.getFullStudent(KEY);
        //Assert        
        assertEquals(expected, result);
        Mockito.verify(mock, times(1)).getFullStudent(KEY);
    }
    ...
    public void testGetFullStudentNotExist() throws Exception {
        ...
        StudentRepository repository = new StudentRepository(mock);
        //Action
        StudentDto result = repository.getFullStudent(patrick.getKey());
        //Assert        
        assertNull(result);
        Mockito.verify(mock, times(1)).getFullStudent(patrick.getKey());
    }
    ...
    public void testGetFullStudentIncorrectParameter() throws Exception {
        ...
        assertThrows(RepositoryException.class, () -> {
            //Action
            repository.getFullStudent(incorrect);
        });
        Mockito.verify(mock, times(1)).getFullStudent(null);
    }
}
